...
@onready var grid: Node2D = get_tree().get_first_node_in_group("Map")
...
var  floating_text = preload("res://assets/scenes/enemy/floating_text.tscn")
...
func _ready():
	# 从monster_information节点获取属性
	max_health = monster_information.max_health
	health = monster_information.max_health
	power = monster_information.power
	speed = monster_information.speed
	update_health_bar()
...
func find_closest_tower() -> Node2D:
	...
	var available_towers: Array = get_tree().get_nodes_in_group("Tower")
