...
func _on_gui_input(event):
	if event is InputEventMouseMotion and event.button_mask == 1:
		...
		var can_place = tile_data and tile_data.get_custom_data("towerable") and !has_tower(new_tile_pos)
		...
		if current_highlighted_tile:
			...
			if tile_data and tile_data.get_custom_data("towerable") and !has_tower(current_highlighted_tile):
