...
@onready var tower = preload("res://assets/scenes/tower/base_tower.tscn")
...
func _on_gui_input(event):
	if event is InputEventMouseMotion and event.button_mask == 1:
		...
		var new_obstruction_pos = map_layer.obstruction_layer.local_to_map(mouse_pos)
		...
		if not preview_tower:
			preview_tower.add_to_group("active_towers")
			preview_tower = tower.instantiate()
	...
	# 修复滚动键也可以放置塔
		...
		current_highlighted_tile = map_layer.base_layer.local_to_map(mouse_pos1)
		current_obstruction_tile = map_layer.obstruction_layer.local_to_map(mouse_pos1)
			...
			if can_place_all:
				...
				for x in range(0, temp_tower.size.x):
					for y in range(0, temp_tower.size.y):
						...
						map_layer.tower_layer.set_cell(pos, 0,Vector2i(27,12))  # 使用适当的格子ID
